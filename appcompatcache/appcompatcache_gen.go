...
type AppCompatibilityProfile struct {
    ...
    Off_Win10Header_HeaderSize int64
    Off_Win10Header_NumberOfEntries int64
    Off_A___ int64
}
...
func NewAppCompatibilityProfile() *AppCompatibilityProfile {
    ...
    self := &AppCompatibilityProfile{8,12,14,0,0,40,0,36,0}
    return self
}
...
func (self *AppCompatibilityProfile) Win10Header(reader io.ReaderAt, offset int64) *Win10Header {
    return &Win10Header{Reader: reader, Offset: offset, Profile: self}
}

func (self *AppCompatibilityProfile) A(reader io.ReaderAt, offset int64) *A {
    return &A{Reader: reader, Offset: offset, Profile: self}
}
...
type Win10Header struct {
    Reader io.ReaderAt
    Offset int64
    Profile *AppCompatibilityProfile
}

func NewWin10Header(reader io.ReaderAt) *Win10Header {
    self := &Win10Header{Reader: reader}
    return self
}

func (self *Win10Header) Size() int {
    return 48
}

func (self *Win10Header) HeaderSize() uint32 {
   return ParseUint32(self.Reader, self.Profile.Off_Win10Header_HeaderSize + self.Offset)
}

func (self *Win10Header) NumberOfEntries() uint32 {
   return ParseUint32(self.Reader, self.Profile.Off_Win10Header_NumberOfEntries + self.Offset)
}
func (self *Win10Header) DebugString() string {
    result := fmt.Sprintf("struct Win10Header @ %#x:\n", self.Offset)
    result += fmt.Sprintf("HeaderSize: %#0x\n", self.HeaderSize())
    result += fmt.Sprintf("NumberOfEntries: %#0x\n", self.NumberOfEntries())
    return result
}
...
func NewA(reader io.ReaderAt) *A {
    self := &A{Reader: reader}
    return self
}

func (self *A) Size() int {
    return 0
}

func (self *A) __() uint64 {
    return ParseUint64(self.Reader, self.Profile.Off_A___ + self.Offset)
}
func (self *A) DebugString() string {
    result := fmt.Sprintf("struct A @ %#x:\n", self.Offset)
    result += fmt.Sprintf("__: %#0x\n", self.__())
    return result
}
